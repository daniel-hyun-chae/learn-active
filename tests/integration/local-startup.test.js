...
test('local startup scripts', () => {
  ...
  assert.ok(pkg.scripts['hooks:install'])
  assert.ok(pkg.scripts['validate:lockfile'])
  ...
  const installHooks = read('scripts/install-git-hooks.mjs')
  ...
  assert.ok(installHooks.includes('core.hooksPath'))
  assert.ok(installHooks.includes('.githooks'))
  assert.ok(preCommitHook.includes('pnpm validate:lockfile'))
