...
3) Summarize: once clear, provide a concise change summary, a high-level plan, and an explicit checklist of tasks. Ask the user to confirm or refine. Do not start implementation until the user explicitly says "implement" (case-insensitive). If the user refines, repeat clarify and summarize.
4) Implementation gate: only when the user says implement:
...
5) Implement: execute tasks and update the Task List as you progress (mark [x] when complete). Keep the file updated as the single source of truth.
6) Testing: always run unit, integration, and e2e tests after implementation.
...
7) Final response: summarize changes, reference the CR file path, list tests run. If tests fail, report failure and leave tasks open.
